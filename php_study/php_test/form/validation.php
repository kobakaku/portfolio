...
function validation($request) {
    ...
    if(empty($request['your_name']) || 20 < mb_strlen($request['your_name'])) {
        $errors[] = '「氏名」は必須です。20字以内で入力してください';
    }
    ...
    if(!empty($request['url'])) {
        if(!filter_var($request['url'], FILTER_VALIDATE_URL)) {
            $errors[] = '「URL」は必須です。正しい形式で入力してください。';
        }
    }
    

    if(!isset($request['gender'])) {
        $errors[] = '「性別」は必須です。チェックボックスにチェックしてください。';
    }
    ...
    if(empty($request['contact']) || 200 < mb_strlen($request['contact'])) {
        $errors[] = '「お問い合わせ内容」は必須です。200字以内で入力してください';
    }

    if(empty($request['caution'])) {
        $errors[] = '注意事項にチェックしてください';
    }
